# Write the full English Streamlit app and requirements.txt for download
app_code_en = r"""# -*- coding: utf-8 -*-
...
def run_pipeline_return_buffers(xl_raw_dict: dict) -> tuple[io.BytesIO, io.BytesIO]:
    ...
    for sheet_name, df in final_data.items():
        ...
        if len(cols) >= 2:
            # Make col0 = original, col1 = mapped (or vice versa, per your preference)
    ...
    for sheet_name, df in xl_raw_dict.items():
        ...
        # Normalize boolean-like text to ✓/✗ markers
        df2 = df2.applymap(
    ...
    # ---- STEP 5: Word Validation Report (across all sheets) ----
    doc = Document()
    ...
    for sheet_name, validated_df in validated_sheets_raw.items():
        # Helper to pick columns by partial (case-insensitive) match
        def pick(col_part: str):
    ...
    # Build final Excel (no styling to keep Cloud compatibility)
...
if uploaded and run_clicked:
    try:
        ...
        if uploaded.name.lower().endswith(".csv"):
            ...
            xl_raw_dict = pd.read_excel(uploaded, sheet_name=None)  # multi-sheet Excel
        ...
        st.download_button(
            "📝 Download Validation Report (Word)",
            data=report_buf,
        ...
        # Optional previews
        st.markdown("---")
        st.caption("Tip: You can re-run with another file without refreshing the page.")
